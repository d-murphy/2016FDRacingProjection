...
Here, the summary statistics are used to simulate a 300 runs for each team in each contest.  To predict a run: 
* First, a random variable is pulled to determine if the run takes place.  If the random variable is less than the ratio of Hits to Appearances, the run takes place by sampling from a triangle distribution.  (This simulates a hit)
* If the random variable was greater than the ration of Hits to Appearances, another random variable is pulled to test if the random variable is less than the ratio of Misses to Appearances.  If this is true, a sample is pulled from a uniform distribution.  (This simulates a Miss, but a completed run)
* If both tests are failed, NA is the result.  (This simulates a NT or OT)
